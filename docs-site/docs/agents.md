...
3. **Volume (required)** — on the Railway **project canvas**: **`+ Add`** → **Volume** → attach to the bot service → **Mount path** = `/app/data` (not `/data`). Redeploy. Logs must show `volume mount=/app/data`, not `no volume detected`.
