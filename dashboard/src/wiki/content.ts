    ...
    blocks: [
      ...
      },
      {
        ...
        items: [
          ...
          { title: "Changes", text: "Pending Git updates + Approve if auto-update is off. On Railway, commit lists come from the GitHub API (not local git).", icon: "refresh" },
      ...
      },
      {
        ...
        items: [
          ...
          "Build pill = release version + deploy branch + git SHA + sync (CURRENT / BEHIND / …). Auto-detects host: PM2/VPS uses local git; Railway/Vercel/etc use platform deploy SHA + GitHub tip for CURRENT. Changes tab lists commits via GitHub API on PaaS (optional GITHUB_TOKEN if the repo is private). Auto on/off switch sits next to the pill (host pulls vs Changes → Approve).",
